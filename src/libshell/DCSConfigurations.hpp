...
struct DCSBasicConfiguration
{
    ...
    void setEdgeDirectors(const TopologyData & topology, std::function<Eigen::Vector3d(const Eigen::Vector3d)> normal_function)
    {
        ...
        setEdgeDirectors(topology, faceNormals, normal_function, true);
    }

    void setEdgeDirectors(const TopologyData & topology, const Eigen::MatrixXd & normal_vectors)
    {
        const Eigen::MatrixXd faceNormals = computeFaceNormals(topology);
        setEdgeDirectors(topology, faceNormals, normal_vectors);
    }

    // last argument is added because compiler cannot differentiate from the following function
    void setEdgeDirectors(const TopologyData & topology, const Eigen::Ref<const Eigen::MatrixXd> faceNormals, std::function<Eigen::Vector3d(const Eigen::Vector3d)> normal_function, const bool isFunction)
    {
...
struct DCSDynamicCurrentConfiguration : DCSCurrentConfiguration
{
    ...
    void init(const Eigen::MatrixXd & vertices_in, const Eigen::VectorXd & edgeDirectors_in, const Eigen::MatrixXd & vVertices_in, const Eigen::VectorXd & vEdgeDirectors_in)
    {
        DCSCurrentConfiguration::init(vertices_in, edgeDirectors_in);
        dyndcsdata.init(vVertices_in, vEdgeDirectors_in);
    }
