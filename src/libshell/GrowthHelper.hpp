...
struct GrowthHelper
{
    ...
    static void computeAbarsIsoGrowth(const tMesh & mesh, const Real growthRate, tVecMat2d & aforms)
    {
        Eigen::VectorXd growthArray = Eigen::VectorXd::Constant(aforms.size(), growthRate);
        computeAbarsIsoGrowth(mesh, growthArray, aforms);
    }
    ...
    static void computeAbarsIsoGrowth(const tMesh & mesh, const Eigen::Ref<const Eigen::VectorXd> growthRates, tVecMat2d & aforms)
    {
        ...
        for(int i=0;i<nFaces;++i)
        {
            ...
            const Eigen::Vector3d dir_p = info.e1.normalized();
            const Eigen::Vector3d dir_o = (info.e2 - dir_p * (info.e2.dot(dir_p))).normalized();
    ...
    static void computeAbarsOrthoGrowth(const tMesh & mesh, const Real growthAngle, const Real growthRate_p, const Real growthRate_o, tVecMat2d & aforms)
    {
        const int nFaces = mesh.getNumberOfFaces();
        Eigen::VectorXd growthAngles = Eigen::VectorXd::Constant(nFaces, growthAngle);
        Eigen::VectorXd growthRates_p = Eigen::VectorXd::Constant(nFaces, growthRate_p);
        Eigen::VectorXd growthRates_o = Eigen::VectorXd::Constant(nFaces, growthRate_o);

        computeAbarsOrthoGrowth(mesh, growthAngles, growthRates_p, growthRates_o, aforms);
    }
...
//          also : e0_new + e1_new + e2_new = 0
    ...
    static void computeBbarsOrthoGrowth(const tMesh & mesh, const Real h, const Real growthAngle, const Real growthCurvature_p, const Real growthCurvature_o, tVecMat2d & aforms_top, tVecMat2d & aforms_bottom)
    {
        const int nFaces = mesh.getNumberOfFaces();

        Eigen::VectorXd growthAngles = Eigen::VectorXd::Constant(nFaces, growthAngle);
        Eigen::VectorXd growthCurvatures_p = Eigen::VectorXd::Constant(nFaces, growthCurvature_p);
        Eigen::VectorXd growthCurvatures_o = Eigen::VectorXd::Constant(nFaces, growthCurvature_o);

        computeBbarsOrthoGrowth(mesh, h, growthAngles, growthCurvatures_p, growthCurvatures_o, aforms_top, aforms_bottom);
    }

    static void computeBbarsOrthoGrowth(const tMesh & mesh, const Real h, const Eigen::Ref<const Eigen::VectorXd> growthAngles, const Eigen::Ref<const Eigen::VectorXd> growthCurvatures_p, const Eigen::Ref<const Eigen::VectorXd> growthCurvatures_o, tVecMat2d & aforms_top, tVecMat2d & aforms_bottom)
    {
        const int nFaces = mesh.getNumberOfFaces();
        const auto & topo = mesh.getTopology();
        const auto & reststate = mesh.getRestConfiguration(); // does not matter which one at this point

        Eigen::VectorXd growthRates_p = 1.0 * growthCurvatures_p * h / 3.0;
        Eigen::VectorXd growthRates_o = 1.0 * growthCurvatures_o * h / 3.0;

        computeAbarsOrthoGrowth(mesh, growthAngles, -growthRates_p, -growthRates_o, aforms_top);
        computeAbarsOrthoGrowth(mesh, growthAngles, growthRates_p, growthRates_o, aforms_bottom);
    }

    static void computeBbarsOrthoGrowthViaBbar(const tMesh & mesh, const Eigen::VectorXd & curvatureAngles, const Eigen::VectorXd & curvatures_p, const Eigen::VectorXd & curvatures_o, tVecMat2d & bforms)
    {
        const int nFaces = mesh.getNumberOfFaces();
        const auto & topo = mesh.getTopology();
        const auto & reststate = mesh.getRestConfiguration(); // does not matter which one at this point

        bforms.resize(nFaces);

        for(int i=0;i<nFaces;++i)
        {
            const TriangleInfo info = reststate.getTriangleInfoLite(topo, i);

            const Real phi = curvatureAngles(i);
            const Real curvature_p = curvatures_p(i);
            const Real curvature_o = curvatures_o(i);
            ...
            const Real b11 = e1_p*e1_p*curvature_p + e1_o*e1_o*curvature_o;
            const Real b12 = e1_p*e2_p*curvature_p + e1_o*e2_o*curvature_o;
            const Real b22 = e2_p*e2_p*curvature_p + e2_o*e2_o*curvature_o;

            bforms[i](0,0) = b11;
            bforms[i](0,1) = bforms[i](1,0) = b12;
            bforms[i](1,1) = b22;
        }
    }
...
// template <typename tMesh>
// struct GrowthIterator
// {
//     // absolute or relative growth
//     const bool absolute;
//     Real g0;
//
//     // store information
...
//     const int nFaces;
//
//     Eigen::VectorXd strain_dirs;
//     Eigen::VectorXd strains_p;
//     Eigen::VectorXd strains_o;
//
//     Eigen::VectorXd curvature_dirs;
//     Eigen::VectorXd curvatures_p;
//     Eigen::VectorXd curvatures_o;
//
//     GrowthIterator(const int nFaces, const Real h, const Eigen::VectorXd & steps, const bool absolute):
//     nFaces(nFaces),
//     h(h),
//     absolute(absolute),
//     steps(steps)
//     {}
//
//     GrowthIterator(const int nFaces, const Real h, const int nSteps, const bool absolute):
//     nFaces(nFaces),
//     h(h),
//     absolute(absolute)
//     {
//         steps = Eigen::VectorXd(nSteps);
//         for (int i = 0; i < nSteps; i++) {
//             steps(i) = (i + 1.0) / nSteps;
//         }
//     }
//
//     // isotropic constant
//     void setTargetStrains(const Real strain) {}
//
//     // orthotropic constant
//     void setTargetStrains(const Real strain_dir, const Real strain_p, const Real strain_o) {}
//
//     // isotropic (inhomogeneous) function of position
//     void setTargetStrains(std::function< Real(Eigen::Vector3d) >) {}
//
//     // orthotropic (inhomogeneous) function of position (dir, p, o)
//     void setTargetStrains(std::function< Eigen::Vector3d(Eigen::Vector3d) >) {}
//
//     // isotropic (inhomogeneous) specification per face
//     void setTargetStrains(const Eigen::VectorXd & strains) {}
//
//     // orthotropic (inhomogeneous) specification per face
//     void setTargetStrains(const Eigen::VectorXd & strain_dirs, const Eigen::VectorXd & strains_p, const Eigen::VectorXd & strains_o) {}
//
//     // isotropic constant curvature
//     void setTargetCurvatures(const Real curvature) {}
//
//     // orthotropic constant curvature
//     void setTargetCurvatures(const Real curvature_dir, const Real curvature_p, const Real curvature_o) {}
//
//     // isotropic (inhomogeneous) curvature as function of position
//     void setTargetCurvatures(std::function< Real(Eigen::Vector3d) >) {}
//
//     // orthotropic (inhomogeneous) curvature as function of position (dir, p, o)
//     void setTargetCurvatures(std::function< Eigen::Vector3d(Eigen::Vector3d) >) {}
//
//     // isotropic (inhomogeneous) curvature specification per face
//     void setTargetCurvatures(const Eigen::VectorXd & curvatures) {}
//
//     // orthotropic (inhomogeneous) curvature specification per face
//     void setTargetCurvatures(const Eigen::VectorXd & curvature_dirs, const Eigen::VectorXd & curvatures_p, const Eigen::VectorXd & curvatures_o) {}
//
//     bool setGrowth(const tMesh & mesh, const int step, tVecMat2d & aforms) {
//         if (step < 0 || steps.rows() <= step) {
//             return false;
//         }
//
//         const auto & topo = mesh.getTopology();
...
//             const TriangleInfo info = reststate.getTriangleInfoLite(topo, i);
//
//             const Real growth_p = std::pow(1.0 + strains_p(i), 2);
//             const Real growth_o = std::pow(1.0 + strains_o(i), 2);
//             const Eigen::Vector3d p(std::cos(strain_dirs(i)), std::sin(strain_dirs(i)), 0.0);
//
//             const Eigen::Vector3d normal = (info.e2).cross(info.e0).normalized();
//             const Eigen::Vector3d o = normal.cross(p).normalized();
//
//             const Real e1_p = (info.e1).dot(p);
//             const Real e1_o = (info.e1).dot(o);
//             const Real e2_p = (info.e2).dot(p);
//             const Real e2_o = (info.e2).dot(o);
//
//             const Real a11 = e1_p * e1_p * growth_p + e1_o * e1_o * growth_o;
//             const Real a12 = e1_p * e2_p * growth_p + e1_o * e2_o * growth_o;
//             const Real a22 = e2_p * e2_p * growth_p + e2_o * e2_o * growth_o;
...
//             // const auto & reststate = mesh.getRestConfiguration(); // does not matter which one at this point
//             //
//             // Eigen::VectorXd growthRates_p = 2.0 * growthCurvatures_p * h / 3.0;
//             // Eigen::VectorXd growthRates_o = 2.0 * growthCurvatures_o * h / 3.0;
//             //
//             // computeAbarsOrthoGrowth(mesh, growthAngles, -growthRates_p, -growthRates_o, aforms_top);
//             // computeAbarsOrthoGrowth(mesh, growthAngles, growthRates_p, growthRates_o, aforms_bottom);
//         } else {
